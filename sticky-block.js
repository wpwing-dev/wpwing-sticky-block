...
(function ($) {
	...
	$(".wp-block-wpwing-sticky-block").each(function (sb) {
		var topSpace = $(this).attr("data-top-space");
		var checkForAdmin = $(this).attr("data-check-for-admin");
		var minWidth = $(this).attr("data-min-width");
		var maxWidth = $(this).attr("data-max-width");
		var pushUp = $(this).attr("data-push-up");
		var zIndex = $(this).attr("data-z-index");

		$(this)
			.addClass("wpwing-sticky-block-" + sb)
			.addClass("sticky-block-original-" + sb)
			.addClass("block-is-not-sticky");

		// === Raise flag is the number of pushup elements is NOT 1 ===
		if (pushUp && $(pushUp).length < 1) {
			// There are no elements on the page with the called selector for the Push-up Element.
			console.warn(
				'STICKY BLOCK DEBUGGER: you selected a Push-up element "' +
					pushUp +
					'" but there is no element on the page with that class/ID.'
			);
			pushUp = "";
			...
			console.warn(
				"STICKY BLOCK DEBUGGER: there are " +
					$(pushUp).length +
	...
	function createPlaceholder(sb) {
		$(".sticky-block-original-" + sb)
			.wrap(
				'<div class="sticky-block-wrapper sticky-block-wrapper-' +
					sb +
					'"></div>'
			)
			.addClass("sticky-block-active-" + sb)
			.before(
				'<div class="sticky-block-placeholder sticky-block-placeholder-' +
					sb +
					'" style="width:0; height:0; margin:0; padding:0; visibility:hidden;"></div>'
			);
	}
	...
	function stickIt(
		...
		if (checkForAdmin == "true" && $("body").hasClass("admin-bar") && viewport > 600) {
		...
		if (
			$(window).scrollTop() >= orgElementTop - topSpace - adminBarHeight &&
			viewport >= minWidth &&
			viewport <= maxWidth
		) {
			...
			for (var importantStyle in orgAssignedStyles) {
				if (orgAssignedStyles[importantStyle] == "inline") {
					...
					assignedStyles +=
						importantStyle + ":" + orgAssignedStyles[importantStyle] + "; ";
				}
			}
			...
			if (
				pushUp &&
				$(window).scrollTop() >
					pushElementTop - topSpace - elementHeight - adminBarHeight
			) {
				stickyTopMargin =
					pushElementTop - topSpace - elementHeight - $(window).scrollTop();
			} else {
			...
			$originalElement
				.removeClass("sticky-block-active-" + sb)
				.removeClass("block-is-not-sticky")
				...
				.css("z-index", zIndex);
			if (
				!$(".sticky-block-placeholder-" + sb).hasClass(
					"sticky-block-active-" + sb
				)
			) {
				$(".sticky-block-placeholder-" + sb)
					.addClass("sticky-block-active-" + sb)
